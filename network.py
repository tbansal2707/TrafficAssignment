...
class Network:
   ...
   def travelTime (self) :
      """
      We have defined a new method to capture both travel times : SPTT and TSTT because they are being used in two separate methods
      """
      
      SPTT=0 #Initialising SPTT
      TSTT=0 #Initialising TSTT
      AON=self.allOrNothing()   #All or nothing flows are required for SPTT
      #Iterating over all the links we have
      for i in self.link:
        SPTT=SPTT+AON[i]*self.link[i].cost   #Looking at flow in case of AON and the multiplying by the cost associated with it
        TSTT=TSTT+self.link[i].flow*self.link[i].cost #Looking at number of people on the link and the multiplying by the cost associated with it
   ...
   def relativeGap(self):
      ...
      """
      SPTT, TSTT = self.travelTime() #Retrieving values from the travel time method
      RG=(TSTT/SPTT)-1    #Defined by formula
      return(RG)
   ...
   def averageExcessCost(self):
      ...
      """
      SPTT, TSTT = self.travelTime() #Retrieving values from the travel time method
      demand=0 #Initialising demand
      for i in self.ODpair:
        demand=demand+self.ODpair[i].demand   #Getting demand through all OD pairs
      AEC=(TSTT-SPTT)/demand    #Defined by formula
      return(AEC)
   ...
   def shiftFlows(self, targetFlows, stepSize):
      ...
      *** IMPORTANT: After updating the flow on a link, you should call its
      ...
      """
      #It is pretty self explanatory, given the details Professor has provided to write the code
      for i in self.link:
        self.link[i].flow=stepSize*targetFlows[i]+(1-stepSize)*self.link[i].flow
   ...
   def FrankWolfeStepSize(self, targetFlows, precision = FRANK_WOLFE_STEPSIZE_PRECISION):
      ...
      """
      """
      Solved using both Bisection and Newton's method, please check below
      """
      #Using Newton's Method
      lambda1=1   #The lambda we will use
      lambda2=0    #Dummy to store current lambda
      while abs(lambda2-lambda1)>precision:
         #Initialising f(Lambda) and its differentiation
         fLambda=0
         fPrimeLambda=0
         
         for i in self.link:
            Flow=self.link[i].flow #Saving value to bring it back again
            #Using the shiftFlows method without calling it because I don't want to shift all the flows for now
            self.link[i].flow=lambda1*targetFlows[i]+(1-lambda1)*self.link[i].flow #Flow changed for x(new)
            self.link[i].updateCost()  #Calculate cost at the x(new)
            self.link[i].updateCostPrime()  #Calculate cost at the x(new)
            fLambda+=self.link[i].cost*(targetFlows[i]-Flow)  
            fPrimeLambda += self.link[i].costPrime * ((targetFlows[i] - Flow)** 2) #This is what you get when you differentiate f(Lambda)
            self.link[i].flow=Flow #Assigning the value back to the flow
         
         if fLambda==0: #We achieved the point we wanted
            break
         
         lambda2 = lambda1
         lambda1 -= fLambda / fPrimeLambda #Formula of newton's method
         lambda1 = min(max(lambda1, 0.0), 1.0)
            
      return (lambda1)

      # #Using Bisection Method
      # lambda1=1    #The upper lambda
      # lambda2=0    #The lower lambda
      # while abs(lambda2-lambda1)>precision:
      #    #Initialising f(Lambda) and its differentiation
      #    fLambda=0
      #    lambdaVal = (lambda1+lambda2)/2 #From Bisection Method
         
      #    for i in self.link:
      #       Flow=self.link[i].flow #Saving value to bring it back again
      #       #Using the shiftFlows method without calling it because I don't want to shift all the flows for now
      #       self.link[i].flow=lambdaVal*targetFlows[i]+(1-lambdaVal)*self.link[i].flow #Flow changed for x(new)
      #       self.link[i].updateCost()  #Calculate cost at the x(new)
      #       fLambda+=self.link[i].cost*(targetFlows[i]-Flow)  
      #       self.link[i].flow=Flow #Assigning the value back to the flow
      ...
      #    if fLambda>0:
      #       lambda1=lambdaVal #Discard lower half
      #    else:
      #       lambda2=lambdaVal  #Discard Upper half
