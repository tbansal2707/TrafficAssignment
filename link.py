class Link:
   ...
   def calculateCostPrime(self):
      vcRatio = self.flow / self.capacity
      # Protect against negative flows, 0^0 errors.
      if vcRatio <= 0:
         return 0 
      travelTime = self.freeFlowTime * self.beta * (self.alpha * pow(vcRatio, self.beta-1)) / self.capacity
      return travelTime
   ...
   def updateCostPrime(self):
      """
      Same as calculateCost, except that the link.cost attribute is updated as well.
      """   
      self.costPrime = self.calculateCostPrime()
